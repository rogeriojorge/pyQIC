            ...
            for spsi in [-1, 1]:
                for nfp in range(1, 4):
                    for j in range(3):
                        ...
                        stel = Qic(rc=[R0], zs=[0], nfp=nfp, nphi=nphi, sG=sG, \
            ...
            for spsi in [-1, 1]:
                for nfp in range(1, 4):
                    for j in range(3):
                        ...
                        s = Qic(rc=[R0], zs=[0], nfp=nfp, nphi=nphi, sG=sG, \
...
class CylindricalCartesianTensorsTests(unittest.TestCase):
    ...
    def test_cylindrical_cartesian_tensors(self):
        ...
        stel = Qic(rc=[Rmajor, 0.], zs=[0, 0.], etabar=1.0, B0=B0, nphi=151)
        ...
        stel = Qic.from_paper(1, nphi=151)
        dBdx_cylindrical = stel.grad_B_tensor_cylindrical
        np.testing.assert_almost_equal(dBdx_cylindrical, dBdx_cylindrical.transpose(1, 0, 2), decimal=4)
        dBdx_cartesian = stel.grad_B_tensor_cartesian()
        np.testing.assert_almost_equal(dBdx_cartesian, dBdx_cartesian.transpose(1, 0, 2), decimal=4)
        ...
        np.testing.assert_almost_equal(dBdxdx_cylindrical_transpose_4, dBdxdx_cylindrical, decimal=4)
        np.testing.assert_almost_equal(dBdxdx_cylindrical_transpose_5, dBdxdx_cylindrical, decimal=4)
        ...
        np.testing.assert_almost_equal(dBdxdx_cartesian_transpose_4, dBdxdx_cartesian, decimal=4)
        np.testing.assert_almost_equal(dBdxdx_cartesian_transpose_5, dBdxdx_cartesian, decimal=4)
