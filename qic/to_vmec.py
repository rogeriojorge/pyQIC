...
def to_vmec(self, filename, r=0.1, params=dict(), ntheta=20, ntorMax=14,firstOrderSurface=False):
    ...
    file_object.write('! Near-axis parameters:  radius r = '+str(r)+', omn_method = '+self.omn_method+', d_over_curvature = '+str(self.d_over_curvature)+'\n')
    file_object.write('! d_svals = '+str(self.d_svals)+', d_cvals = '+str(self.d_cvals)+', B0 = '+str(self.B0_vals)+', k_for_alpha = '+str(self.k_buffer)+'\n')
    file_object.write('! d_over_curvature_svals = '+str(self.d_over_curvature_svals)+', d_over_curvature_cvals = '+str(self.d_over_curvature_cvals)+'\n')
    file_object.write('! nphi = '+str(self.nphi)+', order = '+self.order+', sigma0 = '+str(self.sigma0)+', I2 = '+str(self.I2)+', B0 = '+str(np.mean(self.Bbar))+'\n')
    file_object.write('! omn = '+str(self.omn)+', half_helicity = '+str(self.half_helicity)+', d_propToB = '+str(self.d_propToB)+'\n')
